...
def ms_bfs_based_rpq(
    ...
    while frontier.count_nonzero() != 0:
        next_frontiers = {}
        for symbol in common_symbols:
            next_frontiers[symbol] = frontier @ adj_matrix_nfa.matricies[symbol]

            for idx in range(len(nfa_initial_states)):
                start_idx = idx * dfa_total_states
                end_idx = (idx + 1) * dfa_total_states

                next_frontiers[symbol][start_idx:end_idx] = (
                    transposed_matricies[symbol]
                    @ next_frontiers[symbol][start_idx:end_idx]
                )

        frontier = reduce(lambda x, y: x + y, next_frontiers.values(), frontier)
        frontier = frontier > visited_states
        visited_states += frontier
    ...
    for dfa_final_state in adj_by_regex.final_states:
        for i, nfa_start_state in enumerate(nfa_initial_states):
            for nfa_reached in visited_states.getrow(
                dfa_total_states * i + dfa_final_state
            ).indices:
